...
for url in website_urls:
    ...
    # Append the website name and GIF URLs to the data list
    data.extend([(url, gif_url) for gif_url in gif_urls])

# Create a DataFrame from the data list
df = pd.DataFrame(data, columns=['Website', 'GIF URL'])

# Save the DataFrame to an Excel file
df.to_excel('pdkv.xlsx', index=False)
